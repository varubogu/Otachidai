...
pub async fn handle(
    ...
    with_guild_context(&state.db.guild, guild_id.get(), |txn| {
        Box::pin(async move {
            guild_settings::set_room_list_channel(txn, guild_id.get(), channel_id).await?;
            // The board lives in a new channel, so any previously stored message id
            // is for a message in a different channel and must be cleared.
            guild_settings::set_room_list_message_id(txn, guild_id.get(), None).await
        })
    })
    .await?;

    // Post the initial status board. Best-effort: a failure here must not fail
    ...
    Ok(state.i18n.get_with_args(
        lang,
        &MessageKey::AdminRoomListChannelRegistered,
        Some(&args),
    ))
}
